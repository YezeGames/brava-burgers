/**
 * Carga catálogo y configuración desde Google Sheets (productos + configuracion).
 */
(function (global) {
	const SHEET_ID = global.BRAVA_SHEET_ID || '1s3sZcKRqwpCH8L4N1xfgyba14s_HUC3F43FL5ekOCS0';
	const SHEET_PRODUCTOS = global.BRAVA_SHEET_PRODUCTOS || 'productos';
	const SHEET_CONFIG = global.BRAVA_SHEET_CONFIG || 'configuracion';
	const SHEET_EXTRAS = global.BRAVA_SHEET_EXTRAS || 'extras';
	const SHEET_INGREDIENTES = global.BRAVA_SHEET_INGREDIENTES || 'ingredientes';
	...
	function parseConfigSelectUrl(url) {
	...
	function parseConfigSelectTitulo(url) {
	...
	function labelFromPreguntaConfig(val, fallback) {
		...
		if (s.indexOf('titulo=') !== -1) {
			const t = parseConfigSelectTitulo(s);
			return t || fallback;
		}
	...
	function buildDefaultBravaPieHtml(telefono) {
		...
		return (
			'<div class="brava-footer-content">' +
	...
	var AR_DAY_MAP = { Sun: 0, Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6 };
	...
	function argentinaNowParts() {
		...
		try {
			...
			var tmFmt = new Intl.DateTimeFormat('en-GB', {
				timeZone: 'America/Argentina/Buenos_Aires',
				hour: '2-digit',
				minute: '2-digit',
				hour12: false,
			});
			var wd = wdFmt.format(d);
			var day = AR_DAY_MAP[wd];
			if (day === undefined) day = d.getDay();
			var parts = tmFmt.formatToParts(d);
			var h = 0;
			var min = 0;
			...
			return { day: day, minutes: h * 60 + min };
		} catch (eTz) {
	...
	function buildHorarioResumenSemanal(cfg) {
		var parts = [];
		for (var d = 0; d < 7; d++) {
			var ranges = formatHorarioRanges(getHorarioRaw(cfg, d));
			if (ranges) parts.push(BRAVA_DIAS_NOMBRE[d] + ' ' + ranges);
		}
		return parts.join(' · ');
	}
	...
	function buildMensajeCerradoPopup(cfg) {
		var semanal = buildHorarioResumenSemanal(cfg);
		if (semanal) {
			return '¡Estamos cerrados!<br><br>' + semanal.replace(/ · /g, '<br>');
		}
		var custom =
			configGet(cfg, 'Mensaje si está CERRADO') || configGet(cfg, 'Mensaje si esta CERRADO') || '';
		return (custom || '¡Estamos cerrados!').replace(/<BR>/gi, '<br>');
	}
	...
	function legacyPieToHeroLines(html, telefono) {
		...
		return '<div class="brava-footer-content">' + lines.join('') + '</div>';
	}
	...
	function normalizePieHtml(html, telefono) {
		...
		if (!/BRAVA BURGERS|bravaburgers\.ok|brava-footer-pie|brava-footer-content/i.test(h)) {
			return buildDefaultBravaPieHtml(telefono);
		}
		if (/brava-hero-footer-line|brava-footer-content/i.test(h)) {
			return h;
		}
	...
	/** Menú legacy (3 filas = variantes) → modal extras + quitar sin tocar el Sheet. */
	...
	function applyConfigToGlobals(cfg) {
		...
		global.g_mensaje_cerrado = buildMensajeCerradoPopup(cfg);
		...
		global.g_preguntas = {
			...
			opcionesPago: parseConfigSelectUrl(configGet(cfg, 'Pregunta previa al pedido 5')),
			opcionesTurno: parseConfigSelectUrl(configGet(cfg, 'Pregunta previa al pedido 6')),
		};
	...
	function injectThemeCss() {
		...
		let el = document.getElementById('brava-theme');
		if (!el) {
	...
	global.BravaCatalog = {
		...
		argentinaNowParts: argentinaNowParts,
